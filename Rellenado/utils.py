...
def SimpleSeedFillSimul(width, height, size, vertices, vertices1, xi, yi, xi1, yi1, r, g, b):
	r, g, b = 255 * r, 255 * g, 255 * b
	stack = []
	stack1 = []
	stack.append((xi, yi))
	stack1.append((xi1, yi1))
	while len(stack) > 0:
		x, y = stack.pop()
		x1, y1 = stack1.pop()
		if (color_pixel(width, height, x, y, size) != [r, g, b] and punto_en_poligono(x, y,vertices)) and (color_pixel(width, height, x1, y1, size) != [r, g, b] and punto_en_poligono(x1, y1,vertices1)):
			set_pixel(x, y, r, g, b, size)
			set_pixel(x1, y1, r, g, b, size)			

		# examine surrounding pixels to see if they should be placed onto stack
		if (color_pixel(width, height, x + 1, y, size) != [r, g, b] and punto_en_poligono( x + 1, y,vertices)) and (color_pixel(width, height, x1 + 1, y1, size) != [r, g, b] and punto_en_poligono( x1 + 1, y1,vertices1)):
			stack.append((x + 1, y))
			stack1.append((x1 + 1, y1))			

		if (color_pixel(width, height, x + 1, y + 1, size) != [r, g, b] and punto_en_poligono(x + 1, y + 1,vertices)) and (color_pixel(width, height, x1 + 1, y1 + 1, size) != [r, g, b] and punto_en_poligono(x1 + 1, y1 + 1,vertices1)):
			stack.append((x + 1, y + 1))
			stack1.append((x1 + 1, y1 + 1))
			

		if (color_pixel(width, height, x, y + 1, size) != [r, g, b] and punto_en_poligono(x, y + 1,vertices)) and (color_pixel(width, height, x1, y1 + 1, size) != [r, g, b] and punto_en_poligono(x1, y1 + 1,vertices1)) :
			stack.append((x, y + 1))
			stack1.append((x1, y1 + 1))
